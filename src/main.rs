...
impl TImage {
    fn new(input_image_path: String, output_image_path: String, output_format: String, padding: u16) -> Self {
        Self {
            input_image_path,
            output_image_path,
            output_format,
    ...
    fn resize(&self, width: u16, height:  u16) -> ImageResult<()>{
        ...
        let output_image_file = input_image_file.resize(
            output_width,
            output_height,
            Triangle
        );
        ...
        let image_format = if self.output_format == "png" { ImageFormat::Png } else { ImageFormat::Jpeg };
